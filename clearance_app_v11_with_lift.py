...
# -*- coding: utf-8 -*-
"""
建築限界シミュレーター v11 With Lift版
V10の完全再現 + カント時の建築限界モデル浮き上がり機能
浮き上がり量 = 1900 * sin(atan(cant_mm / 1067))
"""
...
class ClearanceModelV11WithLift:
    """建築限界モデル v11 With Lift - V10計算方式 + 浮き上がり機能"""
    
    def __init__(self):
        """初期化"""
        self.rail_gauge = 1067  # 軌間 (mm)
        self.building_clearance_max_width = 1900  # 建築限界最大幅 (mm)
        
    def calculate_base_clearance_at_height(self, height: float) -> float:
        """高さに対する基本建築限界離れ（V10と同一）"""
        if height < 0:
            return float('inf')
        elif height < 25:
            return 1225
    ...
    def calculate_lift_amount(self, cant_mm: float) -> float:
        """カント時の浮き上がり量計算（V11新機能）
        
        浮き上がり量 = 1900 * sin(atan(cant_mm / 1067))
        
        物理的意味：建築限界の最大幅端点(1900mm)がカント角度で回転した時の垂直上昇成分
        """
        if cant_mm == 0:
            return 0
        cant_angle = math.atan(cant_mm / self.rail_gauge)
        lift_amount = self.building_clearance_max_width * math.sin(cant_angle)
        return lift_amount
    
    def transform_clearance(self, points: List[Tuple[float, float]],
                           cant_mm: float, curve_radius_m: float) -> List[Tuple[float, float]]:
        """建築限界変形（V11: 回転変形 + 浮き上がり）"""
        if not points:
            return []
        ...
        # V11新機能: カント回転変形後の浮き上がり量適用
        # OIRANエクセルでは回転変形後に特定の基準点が177.2924mmの位置に来るよう浮き上がる
        lift_amount = self.calculate_lift_amount(cant_mm)
        if lift_amount > 0:
            # 回転変形後の最低Y座標を取得
            min_y_after_rotation = np.min(coords[:, 1])
            # 浮き上がり量から現在の最低Y座標を引いた分だけ全体を上に移動
            vertical_offset = lift_amount - min_y_after_rotation
            coords[:, 1] = coords[:, 1] + vertical_offset
...
class ExcelAccurateCalculatorV11WithLift:
    """Excel計算方式の完全再現計算器 V11 With Lift（V10計算エンジンと同一）"""
...
class ClearanceAppV11WithLiftUI:
    """建築限界シミュレーターアプリケーション V11 With Lift UI版"""
    
    def __init__(self):
        """初期化"""
        self.root = tk.Tk()
        self.root.title("建築限界シミュレーター V11.0 With Lift - Excel完全再現 + 浮き上がり機能")
        ...
        # 計算エンジン（V11のExcel完全再現方式）
        self.excel_calculator = ExcelAccurateCalculatorV11WithLift()
        
        # 建築限界モデル（V11の浮き上がり対応）
        self.clearance_model = ClearanceModelV11WithLift()
    ...
    def create_display_area(self, parent):
        ...
        # タイトル
        title_label = ttk.Label(display_frame, text="建築限界モデル（V11浮き上がり対応）", font=self.title_font)
    ...
    def setup_initial_graph(self):
        ...
        self.ax.set_title('建築限界シミュレーション v11.0 With Lift', fontsize=14)
    ...
    def display_results(self):
        ...
        # V11新機能: 浮き上がり量表示
        if result['cant_mm'] > 0:
            lift_amount = self.clearance_model.calculate_lift_amount(result['cant_mm'])
            self.result_text.insert(tk.END, f"浮き上がり量: {lift_amount:.1f} mm\n")
    ...
    def update_display(self):
        ...
        # V11新機能: 浮き上がり量の可視化
        if cant > 0:
            lift_amount = self.clearance_model.calculate_lift_amount(cant)
            # 浮き上がり量を示すライン（レール中央から）
            self.ax.plot([0, 0], [0, lift_amount], 'r:', linewidth=2, 
                        label=f'浮き上がり: {lift_amount:.1f}mm')
            # 浮き上がり量のマーカー
            self.ax.plot(0, lift_amount, 'ro', markersize=4)
