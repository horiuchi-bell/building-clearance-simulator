...
class CompleteClearanceApp:
    """建築限界シミュレーター 完全統合版"""
    
    def __init__(self):
        """アプリケーション初期化"""
        self.root = tk.Tk()
        self.root.title("建築限界シミュレーター - 完全版")
        ...
        # 建築限界モデル初期化
        self.clearance_model = BuildingClearanceModel()
    ...
    def update_ui_status(self, clearance_rank: str, clearance_status: str, rank_color: str,
        ...
        if equipment_safe:
            clearance_width = self.clearance_model._get_clearance_width_at_height(
                equipment_y, self.clearance_model.transform_clearance(
                    self.base_clearance, cant_value, curve_radius)
            )
            if clearance_width:
...
def main():
    """メイン関数"""
    print("🏗️ 建築限界シミュレーター（完全統合版）初期化中...")
    
    try:
        app = CompleteClearanceApp()
        app.run()
    except Exception as e:
        print(f"❌ 初期化エラー: {e}")
